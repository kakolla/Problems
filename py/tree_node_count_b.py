...
class Node:
    ...
    def __init__(self, node_id: int):
        ...
        self.children_nodes = []
        self.firstcountMsg = False
        self.seen_count_msgs = set()
    ...
    def receiveMessage(self, from_node_id, message) -> None:
        ...
        if message == 'count':
            if self.firstcountMsg: return # dont double count

            # first msg node receives
            self.count = 1
            self.parent = from_node_id
            self.processed = 0
            self.firstcountMsg = True
            ...
            if from_node_id in self.seen_count_msgs:
                return
            self.seen_count_msgs.add(from_node_id)
